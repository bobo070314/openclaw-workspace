...
def generate_mermaid(diagram: dict) -> str:
    ...
    lines.append("%% Generated by infra-diagram-as-code")
    lines.append("")
...
def main():
    ...
    parser.add_argument(
        "--template", default=None, choices=["aws", "microservices", "cicd"], help="Use a built-in template"
    )
    parser.add_argument(
        "--format", default="mermaid", choices=["mermaid", "json", "ascii"], help="Output format (default: mermaid)"
    )
    ...
    if args.output and not args.dry_run:
        try:
            ...
            safe_print(f"Diagram written to: {args.output}")
        except Exception as e:
            safe_print(f"ERROR writing to {args.output}: {e}")
            sys.exit(1)
    else:
        if args.output:
            safe_print(f"[DRY-RUN] Would write to: {args.output}")
            safe_print("---")
